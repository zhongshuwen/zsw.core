// Copyright (c) 2021 Pink.Network
// Distributed under the LGPL-3 software license, see the accompanying
// file at https://opensource.org/licenses/LGPL-3.0
// Modified by ZhongShuWen from the original source at https://github.com/pinknetworkx/atomicassets-contract/blob/7991ebfdfe75c32e4ab1e37405c116cda1e76ea5/include/zswcoredata.hpp
...
namespace zswcoredata {
    ...
    typedef std::variant <\
        ...
        zswcoredata::INT8_VEC, zswcoredata::INT16_VEC, zswcoredata::INT32_VEC, zswcoredata::INT64_VEC, \
        zswcoredata::UINT8_VEC, zswcoredata::UINT16_VEC, zswcoredata::UINT32_VEC, zswcoredata::UINT64_VEC, \
        zswcoredata::FLOAT_VEC, zswcoredata::DOUBLE_VEC, zswcoredata::STRING_VEC> SERDATA_ATTRIBUTE;

    typedef std::map <std::string, SERDATA_ATTRIBUTE> ATTRIBUTE_MAP;
    ...
    vector <uint8_t> serialize_attribute(const string &type, const SERDATA_ATTRIBUTE &attr) {
        if (type.find("[]", type.length() - 2) == type.length() - 2) {
            ...
            if (std::holds_alternative <INT8_VEC>(attr)) {
                ...
                for (auto child : vec) {
                    SERDATA_ATTRIBUTE child_attr = child;
    ...
    SERDATA_ATTRIBUTE deserialize_attribute(const string &type, vector <const uint8_t>::iterator &itr) {
        if (type.find("[]", type.length() - 2) == type.length() - 2) {
            //Type is an array
